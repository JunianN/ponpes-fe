...
import { AddSarprasFormModal } from "@/components/templates/AddSarprasFormModal";
import { EditSarprasFormModal } from "@/components/templates/EditSarprasFormModal";
...
export function SeeSarpras() {
  useRole("ADMIN");
  const toast = useToast();

  const [refreshSignal, setRefreshSignal] = useState(false);

  const { data: booksData, isLoading } = useFetch("/sarpras", refreshSignal);
  // const books = useMemo(() => data?.data?.books?.reverse() || [], [data]);

  const [books, setBooks] = useState([]);
  useEffect(() => {
    if (!booksData?.data?.sarpras) return;
    setBooks(booksData.data.sarpras.reverse());
  }, [booksData]);
  console.log(books)
  // filters
      ...
      <ConfirmDialog
        ...
        onActionClick={async () => {
          try {
            ...
            await fetcher.delete("/sarpras/" + selectedBook._id);
